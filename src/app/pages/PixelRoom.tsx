...
import { ArrowDown, ArrowLeft, ArrowRight, ArrowUp, CornerDownLeft, List, RotateCcw } from 'lucide-react';
...
export default function PixelRoom({ onList }: { onList: () => void }) {
  ...
  return <main className={`pixel-room ${darkMode ? 'is-night' : 'is-day'}`}>
    <div className="room-topline"><div className="room-mono"><span className="room-live-dot" /> A LITTLE PLACE ON THE INTERNET</div><button className="room-list-button" onClick={onList}><List size={15} />传统列表 <ArrowUpRightGlyph /></button></div>
    ...
    <section className="room-stage" aria-label="可探索的像素房间">
      ...
      {status !== 'ready' && <div className="room-loading" role="status">{status === 'loading' ? <><span className="room-loading-pixel" /><h2>正在点亮小屋</h2><p>摆好书本，泡一杯咖啡…</p></> : <><h2>小屋暂时没能打开</h2><p>可以切换到传统列表继续浏览内容。</p><div><button onClick={() => setRevision(r => r + 1)}>重新进入</button><button onClick={onList}>传统列表</button></div></>}</div>}
...
function ArrowUpRightGlyph() { return <span aria-hidden="true">↗</span>; }
function PixelHeart() {
